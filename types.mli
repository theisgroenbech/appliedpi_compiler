...
and process =
    Nil
  | Par of process * process (* P | Q *)
  | Repl of process * occurrence (* ! P *)
  | Restr of funsymb * new_args * process * occurrence (* new x[args]; P *)
  | Test of term * process * process * occurrence (* if ... then P else Q *)
  | Input of term * pattern * process * occurrence (* in(M, π); P *)
  | Output of term * term * process * occurrence (* out(M, N); P *)
  | Let of pattern * term * process * process * occurrence (* let π = M in P else Q *)
  | LetFilter of binder list * fact * process * process * occurrence (*?*)
  | Event of term  * new_args * process * occurrence (* event M; P *)
  | Insert of term * process * occurrence (* ignore *)
  | Get of pattern * term * process * process * occurrence (* ignore *)
  | Phase of int * process * occurrence (* ignore *)
  | Barrier of int * string option * process * occurrence (* ignore *)
  | AnnBarrier of int * string * funsymb * funsymb * (binder * term) list * process * occurrence (* ignore *)
  | NamedProcess of string * term list * process (* let proc(args) = P *)
